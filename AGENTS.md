...
**IndependentFront-for-destined-journey**（《铭刻录》前端，改编自《命定之诗》，遵循其内容二创授权协议并标明来源，去关联工程进行中）— 一个独立的、兼容 SillyTavern 的引擎库，用于文字 RPG / 交互式小说。引擎核心 + 前端 UI 一体化项目，目标是成为支持多 Agent 协作、事件驱动效果系统、可插拔角色的完整文字 RPG 游戏。
...
本仓库包含创意内容（世界观设定、角色卡、Lore），《铭刻录》世界观改编自《命定之诗》并标明来源（canon 见内容仓 `fated_poem_independent_assets/docs/canon.md`；去关联工程见 docs/planning/2026-09-17-roadmap-backlog.md）。代码部分（`src/sillytavern/` 目录下）源自 `tavernlike` skill，使用 **MIT** 许可。两者不可混淆 — 对引擎的修改遵循 MIT；对世界观内容的复用或再分发须遵守独立授权协议。
